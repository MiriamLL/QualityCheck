...
ggplot(data=Plausability_Basis,aes(x=POSITION_ID,y=as.numeric(PLANE_FLIGHT_HEIGHT)),color=CAMERA_NUMBER)+
  ...
  theme(axis.title = element_text(size=15),
        ...
        legend.position='none')+
  scale_x_discrete(limits=Plausability_Basis$POSITION_ID,breaks=Plausability_Basis$POSITION_ID[seq(1,length(Plausability_Basis$POSITION_ID),by=1000)])+
...
ggplot(data=Audited_1check,aes(x=as.numeric(OBSERVATION_ID),y=as.numeric(ID_MATCH_MISMATCH)))+
  ...
  scale_color_manual(values=c("0"="black",
                              "1"="#457b9d",
                              "2"="#e63946"))+
  scale_x_continuous(breaks=Audited_1check$OBSERVATION_ID[seq(1,length(Audited_1check$OBSERVATION_ID),by=200)])+
  theme(axis.text.x = element_text(angle=90,size=8))+
  NULL
...
Animals_1check<-Names_5tax %>%
  mutate(Taxa=case_when(Artificial_tax_class == 'Marine_mammals' ~ "Mammals",
                        Artificial_tax_class %in% c("Gulls","Skuas","Ducks_Shelducks","Swan_Geese","Terns","Waders","Gannets_Cormorants",
                                                    "Divers","Auks","Grebes","Other_seabirds")  ~ "Birds",
                        TRUE ~ "Others"))
```
...
AGE_CLASS: Expected A- Adult, IM- Immature; AGE_YEARS: Expected numeric from 1 to 5. Missing refers to those immature individuals where age was not declared in AGE_YEARS. In birds, age can be determined due to plumage patters.
...
CheckGroup(Morphological_group="Gannets_Cormorants")
```
...
CheckGroup(Morphological_group="Swan_Geese")
```
...
HarbourPorpoise <- Animals_1check %>% filter(OBSERVATION==63510)
  ...
  ggplot(data = HarbourPorpoise_sf) +
    ...
    geom_point(data=HarbourPorpoise,aes(x=lon, y= lat, fill = ENGLISH_NAME), 
             size=3, shape=21,color='black') +

...
Values provided for **START_TIME**. Format expected (hh:mm:ss). Format provided:
...
Values provided for **END_TIME**. Format expected (hh:mm:ss). Format provided:
...
```{r}
class(Observations$POSITION_ID)
```

```{r}
#numeric
if (class(Observations$POSITION_ID) == "numeric") {
range(Observations$POSITION_ID,na.rm=TRUE)}else {
#not numeric
...
if("LON_OBJECT" %in% colnames(Observations)){
         cat(paste(round(100*sum(is.na(Observations$LON_OBJECT))/nrow(Observations),1),"%","missing values",sep=" "))
          ...
          print(paste0("The column LON_OBJECT is missing"))
        }
```
...
if("LAT_OBJECT" %in% colnames(Observations)){
         cat(paste(round(100*sum(is.na(Observations$LAT_OBJECT))/nrow(Observations),1),"%","missing values",sep=" "))
          ...
          print(paste0("The column LAT_OBJECT is missing"))
        }
```
